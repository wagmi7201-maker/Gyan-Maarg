  ...
  const handleRegister = () => {
    ...
    fetch(`${API_BASE_URL}/signup/`, {
      ...
      body: JSON.stringify({
        ...
        // referralCode can be added later if needed
      }),
    })
      .then(async (response) => {
      ...
      .then(({ response, data }) => {
        ...
        const hasValidAuthData =
          Boolean(data?.tokens?.access) &&
          Boolean(data?.tokens?.refresh) &&
          Boolean(data?.user);

        if (response.ok && data.success && hasValidAuthData) {
          ...
          clearAuthStorage();
          setError(getErrorMessage(data?.error, "Registration failed"));
        }
      })
      .catch(() => {
        setLoading(false);
